...
public class Mp3DAOImpl implements Mp3DAO {
	...
	enum SQL {
		// 모든 SQL문에 세미콜론은 없다
		// 파일 이름에서 UUID 잘라내기
		// select title, filename, substr(filename, instr(filename, '_')+1) from
		// tbl_mp3

		INSERT(
				"INSERT into tbl_mp3(mno, title, dsc, cate, userid, filename, artist, img, albname) values (seq_mp3.nextval, ?, ?, ?, ?, ?, ?, ?, ?)"), SELECT(
				"select * from tbl_mp3 where mno = ?"), UPDATE(
				"update tbl_mp3"
						+ " set title = ?, dsc = ?, cate = ?, userid = ?, filename = ?, rcount = ?, vcount = ?,"
						+ " artist = ?, img = ?, albname = ?"
						+ " where mno = ?"), DELETE(
				"delete tbl_mp3 where mno=? "), 
				ALLSEL(" select rn, title, filename, artist, img, cnt "
						+ " from( select /*+INDEX_DESC(tbl_bbs pk_bbs) */ "
						+ " rownum rn, mno, title, filename,artist,img, count(mno) over() cnt "
						+ " from tbl_mp3 "
						+ " where mno > 0 and rownum <= (ceil((1/10)) * 100 ) + 1) "
